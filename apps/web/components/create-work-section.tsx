  ...
  return (
    <section className="content-card wide-card">
      <div className="section-heading">
        <p>Create</p>
        <h2>从这里开始建立你的作品工作台</h2>
      </div>

      <form action={createWorkAction} className="editor-form-grid">
        ...
        <label className="field-block field-block-wide">
          <span>目标读者</span>
          <textarea name="targetAudience" rows={2} placeholder="一行一个，或用逗号分隔，例如：男频，成长流，群像" />
        </label>
        <label className="field-block field-block-wide">
          <span>商业卖点</span>
          <textarea name="commercialHooks" rows={2} placeholder="一行一个，例如：升级体系、势力博弈、边境求生" />
        </label>
        <label className="field-block field-block-wide">
          <span>硬约束</span>
          <textarea name="hardConstraints" rows={2} placeholder="一行一个，例如：主角不能无代价跨阶" />
        </label>
        <label className="field-block field-block-wide">
          <span>题材边界</span>
          <textarea name="contentWarnings" rows={2} placeholder="一行一个，例如：黑暗世界观、战争描写" />
        </label>
